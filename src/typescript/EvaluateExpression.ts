...
type PairStackSign = [number[], string]

// Define the main calculation function
function calculate(s: string): number | never {
    // Remove all spaces from the input string
    s = trimSpaces(s);
    // Initialize the stack and stack sign pair
    let stack: number[] = [];
    let stackSignPair: PairStackSign[] = [];
    ...
    for (let i: number = 0; i < s.length; i++) {
        ...
        if ((s[i] ?? "").match(/[a-z]/gi)) {
            ...
            if (currentFunction === "getRoot") {
                ...
                if (!currentNumber1.length || !currentNumber2.length || (window as { [key: string]: any })[currentFunction] === undefined || (Number(currentNumber1) !== 3 && Number(currentNumber2) < 0)) {
                    // handleError("Invalid Input!!!");
                    // throw new handleError("Invalid Input!!!")
                }
                // Calculate the function value and add it to the stack
                currentNumber1 = (window as { [key: string]: any })[currentFunction](Number(calculate(currentNumber1)), Number(calculate(currentNumber2)));
            ...
            // If the function is not getRoot, get the argument and call the function
                ...
                // If the input is invalid, return an error
                if (!currentNumber.length || (window as { [key: string]: any })[currentFunction] === undefined) {
                    // handleError("Invalid Input!!!");
                    // return;
                }
                // Calculate the function value and add it to the stack
                currentNumber = (window as { [key: string]: any })[currentFunction](Number(calculate(currentNumber)));
            ...
            let newCurrentNumber: number = Number(currentNumber);
            ...
            let getPair: PairStackSign = stackSignPair.pop()!;
            [stack, sign] = getPair;
    ...
    let acc: number = stack.reduce((acc, curr) => acc += curr, 0)
    return acc;
};